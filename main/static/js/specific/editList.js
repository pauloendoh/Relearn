var editResource = Vue.component('edit-resource', {
    ...
    template: `
        <div :id="'resource-' + resource.position" class='edit-resource-item mt-3 bg-white shadow-sm rounded p-2'>
            <div class='row no-gutters'>
                ...
                <div class='col-9 col-md-10 px-2 edit-resource-info'>
                    <input :id="'resource-title-' + resource.position" v-model="resource.title" type="text"  class="edit-resource-input edit-resource-title-input"  placeholder="Orientação" spellcheck="false"/>
                    <div class="ellipsis">
                        <input :id="'resource-url-' + resource.position" v-model="resource.url" type="text" @paste="$emit('paste-url', $event, resource)" class="edit-resource-input edit-resource-url-input"  placeholder="Url do material" spellcheck="false"/>
                    </div>
                    ...
                    <div class="d-flex align-items-center">
                        <label class="mr-3">
                            <input v-model="resource.estimateMinutes" type="number" style="width: 100px;" />
                            Tempo estimado (em minutos)
                        </label>
                        <label class="pointer">
                            <input type="checkbox" v-model="resource.isPaid" class="pointer" style="position: relative; top: 1px;"/> 
                            Conteúdo pago                
                        </label>
                    </div>
    ...
    mounted: function () {
        ...
        if (resourceIdHash) {
            setTimeout(function () {
                ...
                $(resourceIdHash).addClass('highlighted');
                setTimeout(function () {
                    $(resourceIdHash).removeClass("highlighted");
                }, 1000)
            }, 500)
        ...
        validateChanges: function () {
            for (var resource of this.resources) {
                if (resource.title == "") {
                    ...
                    briefHighlight('#resource-title-' + resource.position);
                    return false;
                }
                else if (resource.url == "") {
                    alert("Insira uma URL");
                    briefHighlight('#resource-url-' + resource.position);
                    return false;
                }
                else if (resource.url.trim().includes(' ')) {
                    alert("URL inválida");
                    briefHighlight('#resource-url-' + resource.position);
                    return false;
                }
